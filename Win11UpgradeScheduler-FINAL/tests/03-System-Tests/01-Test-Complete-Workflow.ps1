# Test Complete Enhanced PSADT v3 Workflow
Write-Host "`n=== LAUNCHING COMPLETE ENHANCED PSADT WORKFLOW ===" -ForegroundColor Cyan
Write-Host "This will run the full Windows 11 Upgrade Scheduler with all enhancements" -ForegroundColor Yellow

# First, let's integrate the enhanced components into the existing PSADT
$basePath = "C:\code\Windows\Phase1-Detection\Win11Scheduler-PSADT-v3"
$enhancedPath = "C:\code\Windows\Win11UpgradeScheduler"

# Copy enhanced components
Write-Host "`nIntegrating enhanced components..." -ForegroundColor Yellow

# 1. Copy enhanced calendar picker
$enhancedPicker = "$enhancedPath\SupportFiles\Show-EnhancedCalendarPicker.ps1"
if (Test-Path $enhancedPicker) {
    Copy-Item -Path $enhancedPicker -Destination "$basePath\SupportFiles\Show-EnhancedCalendarPicker.ps1" -Force
    Write-Host "  [+] Enhanced calendar picker integrated" -ForegroundColor Green
}

# 2. Create Modules directory if it doesn't exist
if (-not (Test-Path "$basePath\SupportFiles\Modules")) {
    New-Item -Path "$basePath\SupportFiles\Modules" -ItemType Directory -Force | Out-Null
}

# 3. Copy scheduler module
$schedulerModule = "$enhancedPath\SupportFiles\Modules\UpgradeScheduler.psm1"
if (Test-Path $schedulerModule) {
    Copy-Item -Path $schedulerModule -Destination "$basePath\SupportFiles\Modules\UpgradeScheduler.psm1" -Force
    Write-Host "  [+] Scheduler module integrated" -ForegroundColor Green
}

# 4. Copy pre-flight checks module
$preflightModule = "$enhancedPath\SupportFiles\Modules\PreFlightChecks.psm1"
if (Test-Path $preflightModule) {
    Copy-Item -Path $preflightModule -Destination "$basePath\SupportFiles\Modules\PreFlightChecks.psm1" -Force
    Write-Host "  [+] Pre-flight checks module integrated" -ForegroundColor Green
}

# 5. Copy wrapper script
$wrapperScript = "$enhancedPath\SupportFiles\ScheduledTaskWrapper.ps1"
if (Test-Path $wrapperScript) {
    Copy-Item -Path $wrapperScript -Destination "$basePath\SupportFiles\ScheduledTaskWrapper.ps1" -Force
    Write-Host "  [+] Scheduled task wrapper integrated" -ForegroundColor Green
}

# Now create an enhanced Deploy-Application script that uses the new components
Write-Host "`nCreating enhanced Deploy-Application script..." -ForegroundColor Yellow
...
# Save the enhanced deployment script
$enhancedDeployPath = "$basePath\Deploy-Application-Enhanced.ps1"
$enhancedDeployScript | Set-Content -Path $enhancedDeployPath -Force
Write-Host "  [+] Enhanced Deploy-Application script created" -ForegroundColor Green

# Launch the complete workflow
Write-Host "`n=== LAUNCHING COMPLETE ENHANCED PSADT WORKFLOW ===" -ForegroundColor Cyan
Write-Host "This will show:" -ForegroundColor Yellow
Write-Host "  1. PSADT Welcome Dialog" -ForegroundColor White
Write-Host "  2. Pre-flight system checks" -ForegroundColor White
Write-Host "  3. Upgrade Information Dialog" -ForegroundColor White
Write-Host "  4. Enhanced Calendar Picker (with Tonight options)" -ForegroundColor White
Write-Host "  5. Schedule confirmation" -ForegroundColor White

Write-Host "`nPress Enter to launch the complete workflow..." -ForegroundColor Cyan
Read-Host

# Change to PSADT directory
Set-Location $basePath

# Launch the enhanced deployment
& powershell.exe -ExecutionPolicy Bypass -File $enhancedDeployPath -DeploymentType Install -DeployMode Interactive