...
"@
        ...
        # Path to Windows 11 installation media
        $setupPath = Join-Path -Path $dirFiles -ChildPath 'setup.exe'
        
        If (Test-Path -Path $setupPath) {
            # Build setup arguments for minimal interaction
            $setupArgs = @(
                '/auto', 'upgrade'
        ...
        Else {
            Write-Log -Message "Windows 11 setup.exe not found at: $setupPath" -Severity 3 -Source $deployAppScriptFriendlyName
            $mainExitCode = 1603
        }
